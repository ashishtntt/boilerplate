...
public class MuleScaffolderCreator {
	...
	/*
	 * @Autowired private CreateMojo createMojo;
	 */

	/*
	 * @Autowired private BuildContext buildContext;
	 */

	/**
	 * Pattern of where to find the spec .raml, .yaml or .yml files.
	 */
	private String[] specIncludes = new String[] { "src/main/resources/api/**/*.yaml",
			"src/main/resources/api/**/*.yml", "src/main/resources/api/**/*.raml", "src/main/resources/api/**/*.json" };
	 ...
	 *//*
		 * @Value(value="${basedir}/${project.name}/") private File specDirectory;
		 */
	...
	public void execute(String artifactId) throws Exception {
		String mulePrjDirectory = baseDir + File.separatorChar + artifactId;

		File specDirectory = new File(mulePrjDirectory);
		File muleXmlDirectory = new File(mulePrjDirectory);
		File muleXmlOutputDirectory = new File(mulePrjDirectory + "/src/main/mule");
		...
		try {
			...
			Scaffolder scaffolder = Scaffolder.createScaffolder(log, muleXmlOutputDirectory, specFiles, muleXmlFiles,
					domainFile, minMuleVersion, muleRuntimeEdition);
			scaffolder.run();
		} catch (IOException e) {
