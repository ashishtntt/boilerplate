...
<#if (runtimeEdition != "EE" && runtimeEdition != "CE")>
	...
	<#assign runtimeEdition = "CE">
		<#--${runtimeEdition}-->
</#if>
	<#assign isMuleEE = runtimeEdition == "EE">	
		<#--${isMuleEE?string("true", "false")} -->
		
	<#assign useSharedHttpListener = !((sharedHttpListener)?has_content || sharedHttpListener.equals("empty"))>
		<#if (!useSharedHttpListener) >
		<#assign configRefName = "httpListenerConfigDefault" >
    		<http:listener-config name="httpListenerConfigDefault">
    			<http:listener-connection host="0.0.0.0" port=${r'"${http.port}"'}/>
    		</http:listener-config>
		<#else>
			<#assign configRefName = sharedHttpListener >
		</#if>
